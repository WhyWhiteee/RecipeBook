...
export const useRecipeIngredientsStore = defineStore('recipeIngredients', () => {
  ...
  async function fetchAll(params = {}) {
    ...
    try {
      ...
      const { data } = await api.get('/recipe-ingredients/', { params: filters.value })
  ...
  async function fetchOne(id) {
    loading.value = true
    try {
      const { data } = await api.get(`/recipe-ingredients/${id}/`)
      currentItem.value = data
      return data
    } catch (error) {
      console.error('Failed to fetch recipe ingredient:', error)
      throw error
    } finally {
      loading.value = false
    }
  ...
  async function create(payload) {
    loading.value = true
    try {
      const { data } = await api.post('/recipe-ingredients/', payload)
      return data
    } catch (error) {
  ...
  async function update(id, payload) {
    loading.value = true
    try {
      const { data } = await api.patch(`/recipe-ingredients/${id}/`, payload)
      return data
    } catch (error) {
